...
const COMMANDS: &[CommandSpec] = commands! {
    ...
    "network" => network_cmd, "{status|set <ipv4_only|prefer_ipv4|prefer_ipv6> <mtu:1280-1500> <udp-timeout:1m|3m|5m|10m|15m|30m>|apply|watch}";
    ...
    "route" => route_cmd, "{list|add-domain <proxy|direct|block|warp> <domain-suffix>|remove-domain <proxy|direct|block|warp> <domain-suffix>|edit-domain <old-target> <new-target> <old-suffix> <new-suffix>|apply}";
    "dns" => dns_cmd, "{status|set <profile>|bootstrap {status|set <system|aliyun|baidu|tencent>}|test [domain]|apply} - profiles: default, cloudflare-{doh|dot|udp}[-direct], google-{doh|dot}[-direct], adguard-doh[-direct], quad9-doh[-direct]";
